...
\begin{frame}\frametitle{Putting Jacobians to Use}
Recall, $(J_1+J_2e^{-\lambda t} -\lambda I)\overrightarrow{v_1}=0$. \\\vspace{2em}
Taking determinants, we find the characteristic polynomial of $(J_1+J_2e^{-\lambda t} -\lambda I)$ evaluated at $M=0$, $C=0$ to be $(\lambda - r + d)(\lambda -\gamma + ge^{-\lambda\tau})=0$.
\end{frame}

\begin{frame}
Hence, $\lambda = r-d>0$ is an eigenvalue with positive real part, implying $(0,0)$ is an unstable equilibrium.
\end{frame}
