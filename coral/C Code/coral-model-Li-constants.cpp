...
//printf("%f\t%f\t%f\t%f\t%f\n",dt,beta,tau,x[0],x[1]);
fprintf(fp,"%f,%f,%f,%i,%f,%f,%f,%f,%f,%f\n",dt,beta,tau,q+1,h,s,1-h-s,x[2],x[3],1-x[2]-x[3]);
return 0;
}
/* ****************************************************************
main
...
double d = 0.44;
double beta = 0.1;
double gNil = ((d*a*r+d*d)*(gamma-a))/(r*r);
double gOne = (gamma*(a+d))/(a+r);
double g = (1/3)*gamma +.5*(gOne-(1/3)*gamma);
double omega = sqrt((r*r*(g*g-gNil*gNil))/(d*d));
double tauNil = (1/omega)*acos(gNil/g);
printf("omegaSquared = %f, tauNil = %f, gNil = %f, gOne = %f, g = %f\n",omega,tauNil,gNil,gOne,g);
double tau = 2*tauNil;
...
for (double h=.24;h<=.24;h=h+0.1)
for (double s=.27;s<=.27;s=s+0.1)
{
dt=0.0001;
while (dt<=0.0001)
{
